...
def main():
    ...
    md.append("> 🔹 **本表只覆盖 wss/wsd(历史截面/序列),其它函数另有独立字段集(未枚举)**:WindPy 共 24 个数据函数,实测 "
              "`wsq`(实时,`rt_` 前缀字段如 `rt_last`)、`wses`/`wsee`(板块聚合,`sec_*_avg` 如 `sec_close_avg`)"
              "各有**独立且互不相通**的字段命名空间(wss 拒 `rt_last`、wsq 拒 `close`),xla 里无对应候选、且属有限小集合,"
              "本表暂未纳入(待真需实时/板块字段时针对性补,探针 `wind_probe_other_funcs.py`)。\n")
