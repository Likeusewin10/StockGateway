...
网关自带 14 个本机工具（命名空间 `agent`；核心 `agent_agent_run`/`_status`/`_result`，另有 `_sessions` 会话列表、`_events`/`_send`/`_close` 流式与常驻会话、`_memory_*` 服务器端记忆 KV，及 **`fs_put`/`fs_get`/`fs_stat` 主机文件传输**），让远端 Agent 在服务器上**异步**起一次性 `claude -p` / `codex exec` 子进程干活、轮询取结果，实现「本地 Agent 指挥服务器 Agent」。异步模型（立即返 `task_id` + 后台线程跑 + 轮询）避开 MCP 同步超时；状态机 `running→done|failed|timeout`。🔴 子进程全自动非交互（`--dangerously-*`），靠两道防线：① 复用网关 X-API-Key（强随机、只发可信机器）；② **cwd 锁死 `AGENT_PROJECT_DIR`**（`config.get_agent_project_dir` 校验覆盖值仍在仓库根内）。引擎限 `claude`/`codex` 白名单 + `shutil.which` 探活，未装/未登录返回 `failed` 不崩。命令模板里 `--` 在 `{prompt}` 前，防 `-` 开头 prompt 被当 flag。第一步不做常驻会话/跨任务上下文（服务器 `CLAUDE.md` 随 prompt 自动加载弥补）。详见 [`docs/MCP中转网关.md`](docs/MCP中转网关.md) 第六节。

**文件传输（fs_put/fs_get/fs_stat，`mcp_gateway/fs_tools.py`）**：让大模型在网关之间搬文件，读写的是**工具执行所在主机**的磁盘 —— hub 上 `agent_fs_put` 写本机、`peer_pc2_fs_put` 写对等机（注册在同一 `agent` FastMCP 实例上，对等机 agent-only 模式暴露裸名，peer 路由零改动自动生效）。二进制安全（`wb`/`rb` 无编码转换）；路径接受 `C:\...` 与 MSYS `/c/...`（归一），拒绝相对路径；单次上限 8MB（`FS_PUT_MAX_BYTES`/`FS_GET_MAX_BYTES`）；大文件分块 = put `mode:"append"` 逐块（配 `expected_size` 防重试重复追加；append 不回整文件 sha256，避免 O(n²)）/ get `offset` 逐块，传完 `fs_stat` 拿整文件 sha256 终验；失败一律 `{ok:false, error}` 不抛异常。与 `agent_run` 同信任模型（不设路径白名单，X-API-Key 兜底 + 写操作 info 日志）。

**多机 Agent 协同（hub + 对等机，2026-07 重构为「哑服务」架构）**：对等机只跑一个**永不需维护的 HTTP 哑执行小服务** [`peer_service.py`](peer_service.py)（纯标准库单文件、不 import 任何 `mcp_gateway`），对外暴露 `/health /exec /task /file /admin/*` 原语；**所有会演进的业务逻辑（命令模板、会话、fs 分块编排、将来的新工具）全在 hub 侧**，对等机零感知。关键切分：hub 拼好完整命令 argv（含 claude 会话 UUID 预生成，argv[0] 留裸引擎名），**argv[0] 的 `shutil.which` 解析与 `Popen encoding=utf-8` 这两个 OS 相关坑收在对等机 `/exec` 里**。hub 侧 [`mcp_gateway/peers.py`](mcp_gateway/peers.py) 按 `MCP_PEERS` 为每台 peer 注册**原生工具** `peer_<name>_agent_run/_status/_result/fs_put/fs_get/fs_stat/svc_health/svc_update`（经 [`mcp_gateway/peer_client.py`](mcp_gateway/peer_client.py) 同步 httpx 调对等机，connect 超时 5s 防宕机拖死 hub），客户端只连 hub 一个入口、工具面不变。对等机侧从此只有：**系统 Python + claude CLI + `peer_service.py` + 两三行 `.env` + frp 隧道**（不需要 venv/fastmcp/克隆仓库/跟 hub 同步代码）。frp 隧道/域名/Caddy 零改动复用（哑服务顶替旧网关同占 8765；哑服务无 fastmcp 的 DNS-rebinding 防护，Caddy 不再需要 `header_up Host localhost`）。`MCP_PEERS` 的 URL 可带旧 `/mcp` 尾巴（`peer_client` 自动剥）。日常维护：改 hub 工具逻辑=只改 hub+重启 hub 网关，对等机不动；极少数改 `peer_service.py` 本体=hub 跑 `scripts/push_peer_service.py`（经 `svc_update` 确定性推送，peer 侧 py_compile 自检+原子替换+延迟重启，不经 git/LLM）。session_id/记忆不跨机；部署手册（可丢给对等机 Claude 自动执行，含 Win/mac 分支）见 [`docs/部署-对等机AgentMCP.md`](docs/部署-对等机AgentMCP.md)。
