...
> 接入日期：2026-06-26。代码已交付并桩测全绿。
>
> 🔴 **现状（2026-07-02）：QMT / 国泰海通 当前完全不支持，暂时忽略。**
> 经确认，**国泰海通（君弘君智所属券商）不支持 QMT 独立交易 API 权限**——这不是「待开通/走流程」，
> 而是券商侧根本不提供，**此路已封死**。实测 `/qmt/health` 返回 `connected:false, connect()=-1`。
> 软件闸门（`QMT_TRADING_ENABLED`）与护栏代码虽已就位，但**在换用支持 QMT API 的券商之前，`/qmt/*`
> 交易端点全部不可用**（真发单会在 `ensure_qmt` 阶段抛 502）。本仓库对 QMT **不再推进**；
> 东财系真实交易的替代路径是另接 EMT（`emt_api`，见下文与 CLAUDE.md）。

## 1. 背景与定位变更

- StockSDK 原定位「只读取数」（EM/iFinD/Wind 三源）。经用户 2026-06-26 / 2026-07-01 决策，
  **全面放开交易/写能力，靠鉴权（API Key）+ 各源总开关兜底**（详见 CLAUDE.md「全面放开交易决策」）。
  君弘君智（= 迅投 QMT，各券商换皮，国君叫「君弘君智」）的 `xtquant` Python 库本机已装，
  暴露 `/qmt/*`，与 `/em /ths /wind /tdx` 并列。
- **当前真实/写能力现状**（非仅 QMT 一家）：
  - **QMT** `/qmt/*`、**TDX** `/tdx/*`：两个**独立券商交易腿**（各自独立总开关 + 默认 dry-run + 四道护栏 + 审计）。
    **QMT 现完全不支持——国泰海通不提供 QMT 独立交易 API 权限，此路封死；TDX 代码+软件闸已就位、卡在券商终端登录。**
  - **Wind** `/wind/*`：`torder`/`tlogon`/`wupf` 等真实交易/写函数经 `/wind/call` 放行，受
    **独立总开关 `WIND_TRADING_ENABLED`**（默认关，关时 503）+ 审计控制（**不再是旧文所述的 403 硬拦截**）。
    ⚠ **但本机 Wind 为数据版、无交易模块（2026-07-02 确认），`torder` 实际无交易权限、不可用**——
    Wind 真实交易这条已划掉。
  - **EM** `/em/*`：`porder`/`pcreate` 是 **Choice 虚拟组合持仓（真实行情 + 模拟持仓，无真实资金）**，
    **产品上无真实券商下单能力**；东财真实交易须另接 EMT（`emt_api`，本仓库尚未接入）。
  - **iFinD** `/ths/*`：数据版 SDK **无任何交易函数**（纯取数）。
