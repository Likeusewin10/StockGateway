...
🔴 交易/调仓类(tlogon/torder/wupf 等)硬拦截 403——本仓库只读取数不交易(定位边界)；
仅对会话管理/异步类做提示性标注。固定端点默认 usedf=True 直接拿 DataFrame。
"""
...
_WIND_TRADING = {"tlogon", "tlogout", "torder", "tcancel", "tquery", "wupf",
                 ...
                 # C 级底层封装（c_tXxx）同样拦截，防止绕过 Python 层拦截
...
def wind_methods(_=Depends(require_key)):
    ...
    for name in sorted(n for n in dir(w) if not n.startswith("_")):
        ...
        if name.lower() in _WIND_TRADING:
            status = "blocked(交易/写操作,定位边界禁止,调用返回 403)"
...
def wind_call(method: str, body: WindCall = WindCall(),
    ...
    if method.lower() in _WIND_TRADING:
        raise HTTPException(403, "禁止调用交易/写操作方法（本服务只读取数）：{}".format(method))
    ...
    with lock:
        try:
            ...
            # 部分方法不接受 usedf 关键字（如日期函数 tdays），退回不带 usedf 重试
            try:
